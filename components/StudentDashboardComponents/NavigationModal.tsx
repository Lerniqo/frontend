...
import ResourceLibrary from "@/components/ResourceLibrary/PrevResourceLibrary";
...
const TeachersPageWrapper = ({
  ...
  onDataLoad?: (teachers: DetailedTeacherProfile[], total: number) => void;
}) => {
  ...
  const transformTeacherProfile = useCallback(
    (teacher: TeacherProfile): DetailedTeacherProfile => {
      ...
      const subjectsTaught = subjects.slice(
        0,
      ...
      return {
        ...
        availability: {
          ...
          nextAvailable:
            Math.random() > 0.5 ? "Available in 2 hours" : undefined,
        },
        ...
        // Additional Features
        badges: [],
        specializations: subjectsTaught.slice(
          0,
  ...
  const loadTeachers = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        const transformedTeachers = response.data.teachers.map(
        ...
        if (onDataLoad) {
          onDataLoad(
            transformedTeachers,
            response.data.total || transformedTeachers.length
          );
        }
      } else {
  ...
  const getExperienceLevelVariant = (
    level: string
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-white/70 max-w-2xl mx-auto">
          Connect with our experienced educators who are passionate about
          helping you succeed in your learning journey.
        </p>
      </div>
              ...
              >
                <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/20 hover:border-purple-400/30 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10 w-full h-full">
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    ...
                    >
                      {teacher.experienceLevel.charAt(0).toUpperCase() +
                        teacher.experienceLevel.slice(1)}
                    </Badge>
                    <Badge
                    ...
                    >
                      {teacher.availability.status.charAt(0).toUpperCase() +
                        teacher.availability.status.slice(1)}
                    </Badge>
                  </div>
...
const NavigationModal: React.FC<NavigationModalProps> = ({
  ...
  const handleTeachersDataLoad = useCallback(
    (teachers: DetailedTeacherProfile[], total: number) => {
